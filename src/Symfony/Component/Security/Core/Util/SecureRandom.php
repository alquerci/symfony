 ...
 */
final class Symfony_Component_Security_Core_Util_SecureRandom implements Symfony_Component_Security_Core_Util_SecureRandomInterface
{
    private $logger;
    private $useOpenSsl;
    private $seed;
     ...
     */
    public function __construct($seedFile = null, Psr_Log_LoggerInterface $logger = null)
    {
    ...
    {
        return include $this->seedFile;
    }

    private function updateSeed()
    {
        if (!$this->seedUpdated && $this->seedLastUpdatedAt < time() - mt_rand(1, 10)) {
            file_put_contents($this->seedFile, sprintf('<?php return %s;', var_export(array($this->seed, microtime(true)), true)));
        }
