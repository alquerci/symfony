...
class Symfony_Component_Security_Tests_Core_Authorization_AccessDecisionManagerTest extends PHPUnit_Framework_TestCase
{
    public function testSupportsClass()
    {
        $manager = new Symfony_Component_Security_Core_Authorization_AccessDecisionManager(array(
    ...
    public function testStrategies($strategy, $voters, $allowIfAllAbstainDecisions, $allowIfEqualGrantedDeniedDecisions, $expected)
    {
        ...
        $manager = new Symfony_Component_Security_Core_Authorization_AccessDecisionManager($voters, $strategy, $allowIfAllAbstainDecisions, $allowIfEqualGrantedDeniedDecisions);
    ...
    protected function getVoters($grants, $denies, $abstains)
    {
        $voters = array();
        for ($i = 0; $i < $grants; $i++) {
            $voters[] = $this->getVoter(Symfony_Component_Security_Core_Authorization_Voter_VoterInterface::ACCESS_GRANTED);
        }
        for ($i = 0; $i < $denies; $i++) {
            $voters[] = $this->getVoter(Symfony_Component_Security_Core_Authorization_Voter_VoterInterface::ACCESS_DENIED);
        }
        for ($i = 0; $i < $abstains; $i++) {
            $voters[] = $this->getVoter(Symfony_Component_Security_Core_Authorization_Voter_VoterInterface::ACCESS_ABSTAIN);
        }
    ...
    {
        $voter = $this->getMock('Symfony_Component_Security_Core_Authorization_Voter_VoterInterface');
