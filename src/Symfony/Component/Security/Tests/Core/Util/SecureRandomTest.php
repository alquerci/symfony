...
class Symfony_Component_Security_Tests_Core_Util_SecureRandomTest extends PHPUnit_Framework_TestCase
{
    /**
     * T1: Monobit test
     *
    ...
    public function testRun($secureRandom)
    {
        ...
        $addRun = new Symfony_Component_Security_Tests_Core_Util_SecureRandomTestClosure($runs);
    ...
    public function getSecureRandoms()
    {
        ...
        $secureRandom = new Symfony_Component_Security_Core_Util_SecureRandom();
        if ($this->hasOpenSsl($secureRandom)) {
            $secureRandoms[] = array($secureRandom);
        }

        // no-openssl with custom seed provider
        $secureRandom = new Symfony_Component_Security_Core_Util_SecureRandom(sys_get_temp_dir().'/_sf2.seed');
    ...
    {
        $ret = $this->readAttribute($secureRandom, 'useOpenSsl');
...
class Symfony_Component_Security_Tests_Core_Util_SecureRandomTestClosure
{
    private $runs;
