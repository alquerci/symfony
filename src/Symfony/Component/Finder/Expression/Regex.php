 ...
 */
class Symfony_Component_Finder_Expression_Regex implements Symfony_Component_Finder_Expression_ValueInterface
{
    const START_FLAG = '^';
    const END_FLAG   = '$';
    const BOUNDARY   = '~';
     ...
     * @param string $expr
     *
     * @return Symfony_Component_Finder_Expression_Regex
     *
    ...
    {
        return Symfony_Component_Finder_Expression_Expression::TYPE_REGEX;
    }
    ...
    {
        $this->_replaceJokersCBreplacement = $replacement;

        $this->pattern = preg_replace_callback('~[\\\\]*\\.~', array($this, '_replaceJokersCB'), $this->pattern);

        return $this;
    }

    private $_replaceJokersCBreplacement;
    public function _replaceJokersCB($subject)
    {
        $subject = $subject[0];
        $replace = 0 === substr_count($subject, '\\') % 2;

        return $replace ? str_replace('.', $this->_replaceJokersCBreplacement, $subject) : $subject;
    }
