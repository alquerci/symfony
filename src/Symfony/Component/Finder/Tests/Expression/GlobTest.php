...
class Symfony_Component_Finder_Tests_Expression_GlobTest extends PHPUnit_Framework_TestCase
{
    /**
    ...
    {
        foreach ($match as $m) {
            $this->assertRegExp(Symfony_Component_Finder_Expression_Expression::create($glob)->getRegex()->render(), $m, '::toRegex() converts a glob to a regexp');
        }

        foreach ($noMatch as $m) {
            $this->assertNotRegExp(Symfony_Component_Finder_Expression_Expression::create($glob)->getRegex()->render(), $m, '::toRegex() converts a glob to a regexp');
        }
    }
