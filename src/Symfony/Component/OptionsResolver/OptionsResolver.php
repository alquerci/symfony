 ...
 */
class Symfony_Component_OptionsResolver_OptionsResolver implements Symfony_Component_OptionsResolver_OptionsResolverInterface
{
    /**
    ...
    {
        $this->defaultOptions = new Symfony_Component_OptionsResolver_Options();
    }
    ...
    public function setOptional(array $optionNames)
    {
        foreach ($optionNames as $key => $option) {
            if (!is_int($key)) {
                throw new Symfony_Component_OptionsResolver_Exception_OptionDefinitionException('You should not pass default values to setOptional()');
            }
    ...
    public function setRequired(array $optionNames)
    {
        foreach ($optionNames as $key => $option) {
            if (!is_int($key)) {
                throw new Symfony_Component_OptionsResolver_Exception_OptionDefinitionException('You should not pass default values to setRequired()');
            }
     ...
     *
     * @throws Symfony_Component_OptionsResolver_Exception_InvalidOptionsException If any of the options has not been defined.
     */
    private function validateOptionsExistence(array $options)
    {
        ...
        if (count($diff) > 0) {
            ...
            throw new Symfony_Component_OptionsResolver_Exception_InvalidOptionsException(sprintf(
     ...
     * @param array $options An list of option names as keys.
     *
     * @throws Symfony_Component_OptionsResolver_Exception_MissingOptionsException If a required option is missing.
     */
    private function validateOptionsCompleteness(array $options)
    {
        ...
        if (count($diff) > 0) {
            ...
            throw new Symfony_Component_OptionsResolver_Exception_MissingOptionsException(sprintf(
     ...
     *
     * @throws Symfony_Component_OptionsResolver_Exception_InvalidOptionsException If any of the values does not match the
    ...
    private function validateOptionValues(array $options)
    {
        foreach ($this->allowedValues as $option => $allowedValues) {
            if (isset($options[$option]) && !in_array($options[$option], $allowedValues, true)) {
                throw new Symfony_Component_OptionsResolver_Exception_InvalidOptionsException(sprintf('The option "%s" has the value "%s", but is expected to be one of "%s"', $option, $options[$option], implode('", "', $allowedValues)));
            }
        }
    }
     ...
     *
     * @throws Symfony_Component_OptionsResolver_Exception_InvalidOptionsException If any of the types does not match the
