 ...
 * $choiceList = new Symfony_Component_Form_Extension_Core_ChoiceList_ChoiceList($choices, $labels);
 * </code>
 *
 * @author Bernhard Schussek <bschussek@gmail.com>
 */
class Symfony_Component_Form_Extension_Core_ChoiceList_ChoiceList implements Symfony_Component_Form_Extension_Core_ChoiceList_ChoiceListInterface
{
    /**
     ...
     *
     * @throws Symfony_Component_Form_Exception_UnexpectedTypeException If the choices are not an array or Traversable.
     */
    public function __construct($choices, array $labels, array $preferredChoices = array())
    {
        if (!is_array($choices) && !$choices instanceof Traversable) {
            throw new Symfony_Component_Form_Exception_UnexpectedTypeException($choices, 'array or Traversable');
        }
     ...
     * @throws Symfony_Component_Form_Exception_InvalidConfigurationException If no valid value or index could be created for a choice.
     */
     ...
     * @param array  $preferredChoices   The preferred choices.
     *
     * @throws Symfony_Component_Form_Exception_InvalidConfigurationException If no valid value or index could be created.
     */
    protected function addChoice(array &$bucketForPreferred, array &$bucketForRemaining, $choice, $label, array $preferredChoices)
    {
        $index = $this->createIndex($choice);

        if ('' === $index || null === $index || !Symfony_Component_Form_FormConfigBuilder::isValidName((string) $index)) {
            throw new Symfony_Component_Form_Exception_InvalidConfigurationException('The index "' . $index . '" created by the choice list is invalid. It should be a valid, non-empty Form name.');
        }

        $value = $this->createValue($choice);

        if (!is_string($value)) {
            throw new Symfony_Component_Form_Exception_InvalidConfigurationException('The value created by the choice list is of type "' . gettype($value) . '", but should be a string.');
        }

        $view = new Symfony_Component_Form_Extension_Core_View_ChoiceView($choice, $value, $label);
