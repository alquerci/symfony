 ...
 */
class Symfony_Component_Form_Extension_Core_DataTransformer_DateTimeToArrayTransformer extends Symfony_Component_Form_Extension_Core_DataTransformer_BaseDateTimeTransformer
{
    private $pad;
    ...
    public function reverseTransform($value)
    {
        ...
        if (!is_array($value)) {
            throw new Symfony_Component_Form_Exception_UnexpectedTypeException($value, 'array');
        }
        ...
        if (isset($value['month']) && !ctype_digit($value['month']) && !is_int($value['month'])) {
            throw new Symfony_Component_Form_Exception_TransformationFailedException('This month is invalid');
        }

        if (isset($value['day']) && !ctype_digit($value['day']) && !is_int($value['day'])) {
            throw new Symfony_Component_Form_Exception_TransformationFailedException('This day is invalid');
        }

        if (isset($value['year']) && !ctype_digit($value['year']) && !is_int($value['year'])) {
            throw new Symfony_Component_Form_Exception_TransformationFailedException('This year is invalid');
        }

        if (!empty($value['month']) && !empty($value['day']) && !empty($value['year']) && false === checkdate($value['month'], $value['day'], $value['year'])) {
            throw new Symfony_Component_Form_Exception_TransformationFailedException('This is an invalid date');
        }
