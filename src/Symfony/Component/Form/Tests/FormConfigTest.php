 ...
 */
class Symfony_Component_Form_Tests_FormConfigTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testNameAcceptsOnlyNamesValidAsIdsInHtml4($name, $accepted)
    {
        ...
        try {
            new Symfony_Component_Form_FormConfigBuilder($name, null, $dispatcher);
            if (!$accepted) {
                $this->fail(sprintf('The value "%s" should not be accepted', $name));
            }
        } catch (Symfony_Component_Form_Exception_UnexpectedTypeException $e) {
