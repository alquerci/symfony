 ...
 */
class Symfony_Component_Form_FormRenderer implements Symfony_Component_Form_FormRendererInterface
{
    const CACHE_KEY_VAR = 'unique_block_prefix';

    /**
     * @var Symfony_Component_Form_FormRendererEngineInterface
     */
    private $engine;
    ...
    public function __construct(Symfony_Component_Form_FormRendererEngineInterface $engine, Symfony_Component_Form_Extension_Csrf_CsrfProvider_CsrfProviderInterface $csrfProvider = null)
    {
        $this->engine = $engine;
     ...
     * {@inheritdoc}
     */
    public function renderBlock(Symfony_Component_Form_FormView $view, $blockName, array $variables = array())
    {
        if (0 == count($this->variableStack)) {
            throw new Symfony_Component_Form_Exception_Exception('This method should only be called while rendering a form element.');
        }
        ...
        if (!$resource) {
            throw new Symfony_Component_Form_Exception_Exception(sprintf('No block "%s" found while rendering the form.', $blockName));
        }
     ...
     * {@inheritdoc}
     */
    public function searchAndRenderBlock(Symfony_Component_Form_FormView $view, $blockNameSuffix, array $variables = array())
    {
