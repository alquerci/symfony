...
class Symfony_Component_BrowserKit_Tests_ResponseTest extends PHPUnit_Framework_TestCase
{
    public function testGetUri()
    {
        $response = new Symfony_Component_BrowserKit_Response('foo');
        $this->assertEquals('foo', $response->getContent(), '->getContent() returns the content of the response');
    }

    public function testGetStatus()
    {
        $response = new Symfony_Component_BrowserKit_Response('foo', 304);
        $this->assertEquals('304', $response->getStatus(), '->getStatus() returns the status of the response');
    }

    public function testGetHeaders()
    {
        $response = new Symfony_Component_BrowserKit_Response('foo', 200, array('foo' => 'bar'));
        $this->assertEquals(array('foo' => 'bar'), $response->getHeaders(), '->getHeaders() returns the headers of the response');
    }

    public function testGetHeader()
    {
        $response = new Symfony_Component_BrowserKit_Response('foo', 200, array(
    ...
    {
        $response = new Symfony_Component_BrowserKit_Response('foo', 304, array('foo' => 'bar'));
    ...
    public function testMagicToStringWithMultipleSetCookieHeader()
    {
        ...
        $response = new Symfony_Component_BrowserKit_Response('foo', 304, $headers);
