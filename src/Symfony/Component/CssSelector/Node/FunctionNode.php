 ...
 */
class Symfony_Component_CssSelector_Node_FunctionNode implements Symfony_Component_CssSelector_Node_NodeInterface
{
     ...
     * Constructor.
     *
     * @param Symfony_Component_CssSelector_Node_NodeInterface $selector The XPath expression
     * @param string        $type
     * @param string        $name
     * @param Symfony_Component_CssSelector_XPathExpr     $expr
     */
    public function __construct($selector, $type, $name, $expr)
    {
        $this->selector = $selector;
        $this->type = $type;
        $this->name = ((is_object($name) && method_exists($name, '__toString')) ? $name->__toString() : $name);
        $this->expr = $expr;
    }
    ...
    public function toXpath()
    {
        ...
        if (in_array($this->name, self::$unsupported)) {
            throw new Symfony_Component_CssSelector_Exception_ParseException(sprintf('The pseudo-class %s is not supported', $this->name));
        }
        $method = '_xpath_'.str_replace('-', '_', $this->name);
        if (!method_exists($this, $method)) {
            throw new Symfony_Component_CssSelector_Exception_ParseException(sprintf('The pseudo-class %s is unknown', $this->name));
        }
     ...
     *
     * @param Symfony_Component_CssSelector_XPathExpr $xpath
     * @param mixed     $expr
     * @param Boolean   $last
     * @param Boolean   $addNameTest
     *
     * @return Symfony_Component_CssSelector_XPathExpr
     */
    ...
    {
        if ($xpath->getElement() == '*') {
            throw new Symfony_Component_CssSelector_Exception_ParseException('*:nth-of-type() is not implemented');
        }
    ...
    protected function _xpath_contains($xpath, $expr)
    {
        ...
        if ($expr instanceof Symfony_Component_CssSelector_Node_ElementNode) {
            $expr = $expr->formatElement();
        }

        // FIXME: lower-case is only available with XPath 2
        //$xpath->addCondition(sprintf('contains(lower-case(string(.)), %s)', Symfony_Component_CssSelector_XPathExpr::xpathLiteral(strtolower($expr))));
        $xpath->addCondition(sprintf('contains(string(.), %s)', Symfony_Component_CssSelector_XPathExpr::xpathLiteral($expr)));
    ...
    {
        if ($s instanceof Symfony_Component_CssSelector_Node_ElementNode) {
            $s = $s->formatElement();
        }
