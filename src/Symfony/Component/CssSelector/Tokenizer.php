 ...
 */
class Symfony_Component_CssSelector_Tokenizer
{
    /**
    ...
    public function tokenize($s)
    {
        ...
        while (true) {
            ...
            if (preg_match('#[+-]?\d*n(?:[+-]\d+)?#A', $s, $match, 0, $pos) && 'n' !== $match[0]) {
                ...
                $tokens[] = new Symfony_Component_CssSelector_Token('Symbol', $sym, $pos);
                $pos += strlen($match[0]);
            ...
            if (in_array($c2, array('~=', '|=', '^=', '$=', '*=', '::', '!='))) {
                $tokens[] = new Symfony_Component_CssSelector_Token('Token', $c2, $pos);
                $pos += 2;
            ...
            if (in_array($c, array('>', '+', '~', ',', '.', '*', '=', '[', ']', '(', ')', '|', ':', '#'))) {
                if (in_array($c, array('.', '#', '[')) && $precedingWhitespacePos > 0) {
                    $tokens[] = new Symfony_Component_CssSelector_Token('Token', ' ', $precedingWhitespacePos);
                }
                $tokens[] = new Symfony_Component_CssSelector_Token('Token', $c, $pos);
                ++$pos;
            ...
            if ('"' === $c || "'" === $c) {
                ...
                $tokens[] = new Symfony_Component_CssSelector_Token('String', $sym, $oldPos);
            ...
            $tokens[] = new Symfony_Component_CssSelector_Token('Symbol', $sym, $oldPos);
     ...
     * @return array
     *
     * @throws Symfony_Component_CssSelector_Exception_ParseException When expected closing is not found
     */
    private function tokenizeEscapedString($s, $pos)
    {
        ...
        while (true) {
            ...
            if (false === $next) {
                throw new Symfony_Component_CssSelector_Exception_ParseException(sprintf('Expected closing %s for string in: %s', $quote, substr($s, $start)));
            }
     ...
     * @return string
     *
     * @throws Symfony_Component_CssSelector_Exception_ParseException When invalid escape sequence is found
     */
    private function unescapeStringLiteral($literal)
    {
        return preg_replace_callback('#(\\\\(?:[A-Fa-f0-9]{1,6}(?:\r\n|\s)?|[^A-Fa-f0-9]))#', create_function('$matches', '
            if ($matches[0][0] == "\\\\" && strlen($matches[0]) > 1) {
                ...
                throw new Symfony_Component_CssSelector_Exception_ParseException(sprintf("Invalid escape sequence %s in string %s", $matches[0], '.var_export($literal, true).'));
            }
        '), $literal);
    }
     ...
     * @return array
     *
     * @throws Symfony_Component_CssSelector_Exception_ParseException When Unexpected symbol is found
     */
    private function tokenizeSymbol($s, $pos)
    {
        ...
        if ($matchStart == $pos) {
            throw new Symfony_Component_CssSelector_Exception_ParseException(sprintf('Unexpected symbol: %s at %s', $s[$pos], $pos));
        }
