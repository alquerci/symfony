...
class Symfony_Component_Yaml_Parser
{
     ...
     *
     * @throws Symfony_Component_Yaml_Exception_ParseException If the YAML is not valid
     */
        ...
        while ($this->moveToNextLine()) {
            ...
            if (preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+?))?\s*$#u', $this->currentLine, $values)) {
                if ($context && 'mapping' == $context) {
                    throw new Symfony_Component_Yaml_Exception_ParseException('You cannot define a sequence item when in a mapping');
                }
                $context = 'sequence';
                ...
                if (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#')) {
                    ...
                    if (isset($values['leadspaces'])
                        && ' ' == $values['leadspaces']
                        && preg_match('#^(?P<key>'.Symfony_Component_Yaml_Inline::REGEX_QUOTED_STRING.'|[^ \'"\{\[].*?) *\:(\s+(?P<value>.+?))?\s*$#u', $values['value'], $matches)
                    ) {
                ...
                if ($context && 'sequence' == $context) {
                    throw new Symfony_Component_Yaml_Exception_ParseException('You cannot define a mapping item when in a sequence');
                }
                $context = 'mapping';

                // force correct settings
                Symfony_Component_Yaml_Inline::parse(null, $exceptionOnInvalidType, $objectSupport);
                try {
                ...
                if (1 === $lineCount || (2 === $lineCount && empty($this->lines[1]))) {
                    try {
                        $value = Symfony_Component_Yaml_Inline::parse($this->lines[0], $exceptionOnInvalidType, $objectSupport);
                    } catch (Symfony_Component_Yaml_Exception_ParseException $e) {
                        $e->setParsedLine($this->getRealCurrentLineNb() + 1);
                        $e->setSnippet($this->currentLine);
     ...
     *
     * @throws Symfony_Component_Yaml_Exception_ParseException When indentation problem are detected
     */
     ...
     * @return mixed  A PHP value
     *
     * @throws Symfony_Component_Yaml_Exception_ParseException When reference does not exist
     */
    private function parseValue($value, $exceptionOnInvalidType, $objectSupport)
    {
        ...
        try {
            return Symfony_Component_Yaml_Inline::parse($value, $exceptionOnInvalidType, $objectSupport);
        } catch (Symfony_Component_Yaml_Exception_ParseException $e) {
            $e->setParsedLine($this->getRealCurrentLineNb() + 1);
            $e->setSnippet($this->currentLine);
