 ...
 */
class Symfony_Component_Config_Util_XmlUtils
{
    /**
    ...
    public static function loadFile($file, $schemaOrCallable = null)
    {
        ...
        if (!$dom->loadXML(file_get_contents($file), LIBXML_NONET | (defined('LIBXML_COMPACT') ? LIBXML_COMPACT : 0))) {
            ...
            throw new InvalidArgumentException(implode("\n", /* static */ self::getXmlErrors($internalErrors)));
        }
        ...
        if (null !== $schemaOrCallable) {
            ...
            if (!$valid) {
                $messages = /* static */ self::getXmlErrors($internalErrors);
                if (empty($messages)) {
    ...
    public static function convertDomElementToArray(DomElement $element, $checkPrefix = true)
    {
        ...
        foreach ($element->attributes as $name => $node) {
            ...
            $config[$name] = /* static */ self::phpize($node->value);
            $empty = false;
        }
        ...
        foreach ($element->childNodes as $node) {
            if ($node instanceof DOMText) {
                ...
                $value = /* static */ self::convertDomElementToArray($node, $checkPrefix);
        ...
        if (false !== $nodeValue) {
            $value = /* static */ self::phpize($nodeValue);
            if (count($config)) {
                $config['value'] = $value;
            } else {
                $config = $value;
