...
class Symfony_Component_ClassLoader_Tests_ClassLoaderTest extends PHPUnit_Framework_TestCase
{
    public function testGetPrefixes()
    {
        $loader = new Symfony_Component_ClassLoader_ClassLoader();
        $loader->addPrefix('Foo', dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
        $loader->addPrefix('Bar', dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
        $loader->addPrefix('Bas', dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
    ...
    {
        $loader = new Symfony_Component_ClassLoader_ClassLoader();
        $loader->addPrefix(null, dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
        $loader->addPrefix(null, dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
        $fallback_dirs = $loader->getFallbackDirs();
        $this->assertEquals(2, count($fallback_dirs));
    }
    ...
    {
        $className = $this->filterClassNameForCurrentPhpVersion($className);

        $loader = new Symfony_Component_ClassLoader_ClassLoader();
        $loader->addPrefix('Namespaced2\\', dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
        $loader->addPrefix('Pearlike2_', dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
    ...
    public function testAddPrefix()
    {
        ...
        $prefixes = $loader->getPrefixes();
        $this->assertArrayHasKey('Foo', $prefixes);
        $this->assertEquals(2, count($prefixes['Foo']));
    }
    ...
    public function testUseIncludePath()
    {
        ...
        set_include_path(dirname(__FILE__).'/Fixtures/includepath' . PATH_SEPARATOR . $includePath);

        $this->assertEquals(dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures'.DIRECTORY_SEPARATOR.'includepath'.DIRECTORY_SEPARATOR.'Foo.php', $loader->findFile('Foo'));
    ...
    public function testLoadClassFromFallback($className, $testClassName, $message)
    {
        ...
        $loader->addPrefix('Namespaced2\\', dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
        $loader->addPrefix('Pearlike2_', dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures');
        $loader->addPrefix('', array(dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures/fallback'));
        $loader->loadClass($testClassName);
        $this->assertTrue(class_exists($className), $message);
    }
    ...
    public function getLoadClassNamespaceCollisionTests()
    {
        return array(
            array(
                array(
                    'NamespaceCollision\\C' => dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures/alpha',
                    'NamespaceCollision\\C\\B' => dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures/beta',
                ),
            ...
            array(
                array(
                    'PrefixCollision_C_' => dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures/alpha',
                    'PrefixCollision_C_B_' => dirname(__FILE__).DIRECTORY_SEPARATOR.'Fixtures/beta',
                ),
    ...
    private function filterClassNameForCurrentPhpVersion($className)
    {
        if (version_compare(phpversion(), '5.3.0', '<')) {
            ...
            if (false !== strpos($className, '\\')) {
                $this->markTestSkipped('Require PHP >=5.3.0');
            }
        }
