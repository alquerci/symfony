...
class Symfony_Component_DependencyInjection_Tests_CrossCheckTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $loaderClass = 'Symfony_Component_DependencyInjection_Loader_'.ucfirst($type).'FileLoader';
        $dumperClass = 'Symfony_Component_DependencyInjection_Dumper_'.ucfirst($type).'Dumper';

        $tmp = tempnam('sf_service_container', 'sf');

        file_put_contents($tmp, file_get_contents(self::$fixturesPath.'/'.$type.'/'.$fixture));

        $container1 = new Symfony_Component_DependencyInjection_ContainerBuilder();
        $loader1 = new $loaderClass($container1, new Symfony_Component_Config_FileLocator());
        $loader1->load($tmp);

        $dumper = new $dumperClass($container1);
        file_put_contents($tmp, $dumper->dump());

        $container2 = new Symfony_Component_DependencyInjection_ContainerBuilder();
        $loader2 = new $loaderClass($container2, new Symfony_Component_Config_FileLocator());
        $loader2->load($tmp);
