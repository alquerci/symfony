 ...
 */
class Symfony_Component_DependencyInjection_Loader_XmlFileLoader extends Symfony_Component_DependencyInjection_Loader_FileLoader
{
    /**
     * Loads an XML file.
     *
     ...
     * Parses parameters
     *
     * @param Symfony_Component_DependencyInjection_SimpleXMLElement $xml
     * @param string           $file
     */
    private function parseParameters(Symfony_Component_DependencyInjection_SimpleXMLElement $xml, $file)
    {
        if (!$xml->parameters) {
            return;
        }
     ...
     * @param string           $file
     */
    private function parseImports(Symfony_Component_DependencyInjection_SimpleXMLElement $xml, $file)
    {
     ...
     * @param string           $file
     */
    private function parseDefinitions(Symfony_Component_DependencyInjection_SimpleXMLElement $xml, $file)
    {
     ...
     *
     * @param string           $id
     * @param Symfony_Component_DependencyInjection_SimpleXMLElement $service
     * @param string           $file
     */
    private function parseDefinition($id, $service, $file)
    {
        if ((string) $service['alias']) {
            ...
            $this->container->setAlias($id, new Symfony_Component_DependencyInjection_Alias((string) $service['alias'], $public));

            return;
        }

        if (isset($service['parent'])) {
            $definition = new Symfony_Component_DependencyInjection_DefinitionDecorator((string) $service['parent']);
        } else {
        ...
        if (isset($service->configurator)) {
            if (isset($service->configurator['function'])) {
                ...
                if (isset($service->configurator['service'])) {
                    $class = new Symfony_Component_DependencyInjection_Reference((string) $service->configurator['service'], Symfony_Component_DependencyInjection_ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE, false);
                } else {
        ...
        foreach ($service->tag as $tag) {
            ...
            foreach ($tag->attributes() as $name => $value) {
                ...
                $parameters[$name] = Symfony_Component_DependencyInjection_SimpleXMLElement::phpize($value);
            }
     ...
     *
     * @return Symfony_Component_DependencyInjection_SimpleXMLElement
     *
     * @throws Symfony_Component_DependencyInjection_Exception_InvalidArgumentException When loading of XML file returns error
     */
    protected function parseFile($file)
    {
        try {
            ...
            throw new Symfony_Component_DependencyInjection_Exception_InvalidArgumentException(sprintf('Unable to parse file "%s".', $file), $e->getCode(), $e);
        }

        $this->validateExtensions($dom, $file);

        return simplexml_import_dom($dom, 'Symfony_Component_DependencyInjection_SimpleXMLElement');
    }
     ...
     * @param string           $file
     */
    private function processAnonymousServices(Symfony_Component_DependencyInjection_SimpleXMLElement $xml, $file)
    {
        $definitions = array();
        $count = 0;
        ...
        if ($element = $dom->documentElement->getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'schemaLocation')) {
            ...
            for ($i = 0, $nb = count($items); $i < $nb; $i += 2) {
                ...
                if (($extension = $this->container->getExtension($items[$i])) && false !== $extension->getXsdValidationBasePath()) {
                    ...
                    if (!is_file($path)) {
                        throw new Symfony_Component_DependencyInjection_Exception_RuntimeException(sprintf('Extension "%s" references a non-existent XSD file "%s"', get_class($extension), $path));
                    }
...
EOF
     ...
     * @param string       $file
     *
     * @throws Symfony_Component_DependencyInjection_Exception_InvalidArgumentException When no extension is found corresponding to a tag
     */
    private function validateExtensions(DOMDocument $dom, $file)
    {
        foreach ($dom->documentElement->childNodes as $node) {
            ...
            if (!$this->container->hasExtension($node->namespaceURI)) {
                $extensionNamespaces = array_filter(array_map(create_function ('$ext', 'return $ext->getNamespace();'), $this->container->getExtensions()));
                throw new Symfony_Component_DependencyInjection_Exception_InvalidArgumentException(sprintf(
     ...
     * @param Symfony_Component_DependencyInjection_SimpleXMLElement $xml
     */
    private function loadFromExtensions(Symfony_Component_DependencyInjection_SimpleXMLElement $xml)
    {
    ...
    {
        return Symfony_Component_Config_Util_XmlUtils::convertDomElementToArray($element);
    }
}
