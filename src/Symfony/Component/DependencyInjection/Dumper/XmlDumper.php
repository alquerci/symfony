 ...
 * @api
 */
class Symfony_Component_DependencyInjection_Dumper_XmlDumper extends Symfony_Component_DependencyInjection_Dumper_Dumper
{
    /**
     * @var DOMDocument
     */
    ...
    private function addService($definition, $id, DOMElement $parent)
    {
        ...
        if (Symfony_Component_DependencyInjection_ContainerInterface::SCOPE_CONTAINER !== $scope = $definition->getScope()) {
        ...
        if ($callable = $definition->getConfigurator()) {
            ...
            if (is_array($callable)) {
                $configurator->setAttribute($callable[0] instanceof Symfony_Component_DependencyInjection_Reference ? 'service' : 'class', $callable[0] instanceof Symfony_Component_DependencyInjection_Reference ? $callable[0]->__toString() : $callable[0]);
    ...
    private function addServiceAlias($alias, $id, DOMElement $parent)
    {
        ...
        $service->setAttribute('alias', $id->__toString());
        if (!$id->isPublic()) {
            $service->setAttribute('public', 'false');
        }
        $parent->appendChild($service);
    ...
    private function convertParameters($parameters, $type, DOMElement $parent, $keyAttribute = 'key')
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                $element->setAttribute('id', (string) $value->__toString());
                $behaviour = $value->getInvalidBehavior();
                if ($behaviour == Symfony_Component_DependencyInjection_ContainerInterface::NULL_ON_INVALID_REFERENCE) {
                    $element->setAttribute('on-invalid', 'null');
                } elseif ($behaviour == Symfony_Component_DependencyInjection_ContainerInterface::IGNORE_ON_INVALID_REFERENCE) {
     ...
     * @return string
     *
     * @throws Symfony_Component_DependencyInjection_Exception_RuntimeException When trying to dump object or resource
     */
    public static function phpToXml($value)
    {
        switch (true) {
            ...
            case false === $value:
                return 'false';
            case $value instanceof Symfony_Component_DependencyInjection_Parameter:
