...
class Symfony_Component_DomCrawler_Tests_FormTest extends PHPUnit_Framework_TestCase
{
    public static function setUpBeforeClass()
    {
        // Ensure that the private helper class FormFieldRegistry is loaded
        class_exists('Symfony_Component_DomCrawler_Form');
    }
    ...
    public function testConstructorThrowsExceptionIfTheNodeHasNoFormAncestor()
    {
        ...
        try {
            $form = new Symfony_Component_DomCrawler_Form($nodes->item(0), 'http://example.com');
        ...
        try {
            $form = new Symfony_Component_DomCrawler_Form($nodes->item(1), 'http://example.com');
    ...
    public function testConstructor($message, $form, $values)
    {
        ...
        $this->assertEquals(
            $values,
            array_map(create_function('$field', '
                    $class = get_class($field);

                    return array(substr($class, strrpos($class, "_") + 1), $field->getValue());
                '),
                $form->all()
            ),
    ...
    public function testGetFormNode()
    {
        ...
        $form = new Symfony_Component_DomCrawler_Form($dom->getElementsByTagName('input')->item(0), 'http://example.com');
    ...
    public function testGetBaseUri()
    {
        ...
        $nodes = $dom->getElementsByTagName('input');
        $form = new Symfony_Component_DomCrawler_Form($nodes->item($nodes->length - 1), 'http://www.foo.com/');
    ...
    public function testGet()
    {
        ...
        $this->assertEquals('Symfony_Component_DomCrawler_Field_InputFormField', get_class($form->get('bar')), '->get() returns the field object associated with the given name');
    ...
    public function testAll()
    {
        ...
        $this->assertEquals('Symfony_Component_DomCrawler_Field_InputFormField', get_class($fields['bar']), '->all() return an array of form field objects');
    }
    ...
    {
        $registry = new Symfony_Component_DomCrawler_FormFieldRegistry();
    ...
    {
        $field = $this->getMock(
            'Symfony_Component_DomCrawler_Field_FormField',
            array('getName', 'getValue', 'setValue', 'initialize'),
            array(),
            '',
            false
        );
    ...
    protected function createForm($form, $method = null, $currentUri = null)
    {
        ...
        return new Symfony_Component_DomCrawler_Form($nodes->item($nodes->length - 1), $currentUri, $method);
    }
}
