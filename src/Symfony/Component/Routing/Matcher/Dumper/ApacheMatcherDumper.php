 ...
 */
class Symfony_Component_Routing_Matcher_Dumper_ApacheMatcherDumper extends Symfony_Component_Routing_Matcher_Dumper_MatcherDumper
{
    /**
     * Dumps a set of Apache mod_rewrite rules.
     *
     ...
     */
    private function getRouteMethods(Symfony_Component_Routing_Route $route)
    {
        $methods = array();
        if ($req = $route->getRequirement('_method')) {
            $methods = explode('|', strtoupper($req));
