 ...
 */
class Symfony_Bundle_FrameworkBundle_Controller_RedirectController extends Symfony_Component_DependencyInjection_ContainerAware
{
    /**
    ...
    {
        if ('' == $route) {
            return new Symfony_Component_HttpFoundation_Response(null, $permanent ? 410 : 404);
        }

        $attributes = $this->container->get('request')->attributes->get('_route_params');
        unset($attributes['route'], $attributes['permanent']);

        return new Symfony_Component_HttpFoundation_RedirectResponse($this->container->get('router')->generate($route, $attributes, Symfony_Component_Routing_Generator_UrlGeneratorInterface::ABSOLUTE_URL), $permanent ? 301 : 302);
    }
    ...
    public function urlRedirectAction($path, $permanent = false, $scheme = null, $httpPort = null, $httpsPort = null)
    {
        ...
        if (parse_url($path, PHP_URL_SCHEME)) {
            return new Symfony_Component_HttpFoundation_RedirectResponse($path, $statusCode);
        }
        ...
        return new Symfony_Component_HttpFoundation_RedirectResponse($url, $statusCode);
    }
}
